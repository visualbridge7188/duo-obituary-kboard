...
$search_keyword = isset($_GET['keyword']) ? sanitize_text_field(wp_unslash($_GET['keyword'])) : '';
$items = array();
...
<div id="duo-obituary-list" class="duo-obituary duo-obituary-list">
	<div class="duo-obituary-list-header">
		...
		<form class="duo-obituary-search" method="get" action="<?php echo esc_url($url->toString()) ?>">
			...
			<input type="search" name="keyword" value="<?php echo esc_attr($search_keyword) ?>"
				placeholder="고인명 또는 상주명 검색">
	...
	<div class="duo-obituary-table-wrap">
		<table class="duo-obituary-table">
			...
			<tbody>
				<?php if (!$items): ?>
					<tr>
						<td colspan="8" class="duo-obituary-empty">현재 부고 소식이 없습니다.</td>
					</tr>
				<?php endif ?>
				<?php foreach ($items as $content): ?>
					...
					$td_style = $board->isAdmin() ? ' style="border-bottom: none;"' : '';
					?>
					<tr class="<?php echo $is_expired ? 'is-expired' : '' ?>">
						<td<?php echo $td_style ?>><?php echo duo_obituary_display($content, 'affiliation') ?></td>
						<td class="duo-obituary-name"<?php echo $td_style ?>>
							<a
								href="<?php echo esc_url($document_url) ?>"><?php echo duo_obituary_display($content, 'deceased_name') ?></a>
							<?php if ($is_expired): ?><span class="duo-obituary-badge">지난 부고</span><?php endif ?>
						</td>
						<td class="duo-obituary-ellipsis"<?php echo $td_style ?>><a
								href="<?php echo esc_url($document_url) ?>"><?php echo duo_obituary_display($content, 'chief_mourner') ?></a>
						</td>
						<td<?php echo $td_style ?>><?php echo duo_obituary_datetime_display($content, 'death_date') ?></td>
						<td<?php echo $td_style ?>><?php echo duo_obituary_datetime_display($content, 'coffin_date') ?></td>
						<td<?php echo $td_style ?>><?php echo duo_obituary_datetime_display($content, 'funeral_date') ?></td>
						<td class="duo-obituary-ellipsis"<?php echo $td_style ?>><?php echo duo_obituary_display($content, 'place') ?></td>
						<td class="duo-obituary-ellipsis"<?php echo $td_style ?>><?php echo duo_obituary_display($content, 'burial_place') ?></td>
					</tr>
					<?php if ($board->isAdmin()): ?>
					<tr class="duo-obituary-admin-view <?php echo $is_expired ? 'is-expired' : '' ?>">
						<td colspan="8" style="text-align:right; font-size:12px; color:#888; height:auto; padding:4px 14px;">
							조회수: <?php echo intval($content->view) ?>
						</td>
					</tr>
					<?php endif; ?>
				<?php endforeach ?>
	...
	<div class="duo-obituary-mobile-list">
		...
		<?php foreach ($items as $content): ?>
			...
			$is_expired = duo_obituary_is_expired($content);
			?>
			<a class="duo-obituary-mobile-item <?php echo $is_expired ? 'is-expired' : '' ?>"
				...
				<div class="duo-obituary-mobile-meta">
					...
					<span>장례식장 <?php echo duo_obituary_display($content, 'place') ?></span>
				</div>
				<?php if ($board->isAdmin()): ?>
				<div class="duo-obituary-mobile-admin" style="text-align: right; font-size: 12px; color: #888; margin-top: 6px;">
					조회수: <?php echo intval($content->view) ?>
				</div>
				<?php endif; ?>
			</a>
	...
	<?php if ($board->isWriter()): ?>
		<div class="duo-obituary-control">
			<div></div>
			<div class="right"><a href="<?php echo esc_url($url->getContentEditor()) ?>"
					class="duo-obituary-button primary">부고 등록</a></div>
		</div>
	<?php endif ?>
</div>
<?php
